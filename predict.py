# USAGE
# python predict.py --model cadet/experiments/exported_model/frozen_inference_graph.pb \
# 	--labels cadet/records/classes.pbtxt \
# 	--image trials \
# 	--num-classes 2
	...
	with tf.Session(graph=model) as sess:
		...
		for file in sorted(os.listdir(args["image"])):
			...
			for (box, score, label) in zip(boxes, scores, labels):
				...
				endY = int(endY * H)
				box = np.array([box])
				pd.DataFrame(box).to_csv("box_results.csv")
				scores = np.array(scores)
				pd.DataFrame(scores).to_csv("score_results.csv")
	
				# draw the prediction on the output image
				label = categoryIdx[label]
				idx = int(label["id"]) - 1
				label = "{}: {:.5f}".format(label["name"], score)
				cv2.rectangle(output, (startX, startY), (endX, endY),
					COLORS[idx], 2)
				y = startY - 10 if startY - 10 > 10 else startY + 10
				cv2.putText(output, label, (startX, y),
					cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLORS[idx], 1)
				cv2.putText(output, file, ( 100, 80 ),
					cv2.FONT_HERSHEY_SIMPLEX, 2, COLORS[idx], 1)
